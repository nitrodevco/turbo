    ...
    public class PathFinder : IPathFinder
    {
        ...
        private IList<PathFinderNode> CalculatePathFinderNode(IRoomObject roomObject, IPoint location)
        {
            ...
            while (openNodes.Count > 0)
            {
                ...
                if (_punishChangeDirection) horizon = (currentNode.Location.X - currentNode.LocationParent.X);
                ...
                foreach (IPoint walkingPoint in walkingPoints)
                {
                    ...
                    {
                        if((tempNode.Location.X - currentNode.Location.X) != 0)
                        {
                            if (horizon == 0) gCost += 20;
                        }

                        if ((tempNode.Location.Y - currentNode.Location.Y) != 0)
                        {
                            if (horizon != 0) gCost += 20;
                        }
                    }
